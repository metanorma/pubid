...
# `number` attribute — and the regression net for 489 identifiers that used to
...
# the shared base and never set the inherited `number`, so all 3,619 parseable
# fixture ids keyed "".
#
# WORSE, 489 of them never had a code to key on. They parsed, and rendered as
# nonsense: all 320 Errata produced the identical string "ASHRAE Standard
# Errata" (with a double space, the BIPM Declaration corruption signature) and
# the addendum forms produced "ASHRAE Standard " or "… to ASHRAE Standard
# -2024". Two independent causes, triaged over the whole corpus:
#
#   441 ids — the BUILDER. The grammar captured everything; `build_errata` and
...
#             five other call sites did not.
#    48 ids — the GRAMMAR. rule(:code_with_year) matched the code digits but
...
module AshraeIndexKeySpec
  ...
  RECOVERED = {
    # builder: base-wrapped errata (318 ids)
    ...
    # builder: addendum (117 ids)
    "ANSI/ASHRAE/IES Addendum aj to ANSI/ASHRAE/IES Standard 90.1-2022" => "90.1",
    # builder: publisher_base_addendum (6 ids)
    "ASHRAE Addendum e to ASHRAE Guideline 28-2016 (December 3, 2019)" => "28",
    # grammar: code_with_year never captured the code (48 ids)
  ...
  FIXTURE_LINES = Dir
    .glob(File.join(__dir__, "../../fixtures/ashrae/**/*.txt"))
...
RSpec.describe "Pubid::Ashrae index key (root.number)" do
  ...
  # "Interpretations for …" reference never reaches the interpretation grammar
  # — something earlier in the alternation claims it — so it builds a plain
  # Standard and the marker is lost entirely. The result is an identifier
  # IDENTICAL to the standard it interprets: same class, same to_s, same hash,
  # same slug. Verified on the main baseline: all 51 interpretation rows
  # already behaved this way, so this branch neither caused nor worsened it.
  #
  # It is pinned rather than fixed because it is a fourth, separate defect
  # needing grammar-ordering work, and because Identifiers::Interpretation is
  # consequently unreachable — which is also why it is the one supplement type
  # without an mr_supplement_suffix. See hand-off
  ...
  describe "the whole fixture corpus" do
    it "parses a corpus worth sweeping" do
      expect(AshraeIndexKeySpec.parsed_corpus.size).to be >= 3_619
    end
    ...
    # 320 Errata all rendered as the identical "ASHRAE Standard  Errata".
    ...
    # to_slug is an output FILENAME. Before this change ALL 3,619 ids shared the
