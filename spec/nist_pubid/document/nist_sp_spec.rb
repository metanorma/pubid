RSpec.describe Pubid::Nist::Identifier do
  ...
  context "NIST.SP.800-63ver1.0.2" do
    let(:original_pubid) { "NIST.SP.800-63ver1.0.2" }
    let(:short_pubid) { "NIST SP 800-63ver1.0.2" }
  ...
  context "NIST SP 800-189 IPD" do
    let(:original_pubid) { "NIST SP 800-189 IPD" }
    let(:short_pubid) { "NIST SP 800-189 ipd" }
  ...
  context "NIST SP 800-189 IPD spa" do
    let(:original_pubid) { "NIST SP 800-189 IPD spa" }
    let(:short_pubid) { "NIST SP 800-189 ipd spa" }

    it "has correct iteration" do
      expect(subject.stage.to_s).to eq("ipd")
    end

    it "has correct translation" do
      expect(subject.translation).to eq("spa")
    end
